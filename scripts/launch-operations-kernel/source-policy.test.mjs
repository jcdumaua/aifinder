...
await check("actual candidate source policy", async () => {
  ...
  assert.equal(result.live_routes, 3);
  assert.equal(result.live_entrypoints, 3);
});
...
if (failures.length > 0) {
  ...
  console.log(
    `PASS_LAUNCH_OPERATIONS_SOURCE_POLICY assertions=${assertions} mutations=169 network=0 live_routes=3 legacy_imports=0 failures=0 internal_failures=0`,
  );
}
