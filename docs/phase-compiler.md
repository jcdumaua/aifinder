...
### Optional bounded inspection contract

An authored phase may include the optional top-level `inspection_contract` only for a zero-effect, read-only question set. P01-P03 omit the field and retain their prior canonical behavior. P04 exercises version 1 with mode `READ_ONLY_QUESTION_SET`, 15 ordered questions, six ordered CCR output sections, and eight ordered claim boundaries.

The contract is closed and bounded. Question IDs use `Q00` form, section IDs use `S00` form, answer kinds and claim boundaries are fixed enums, the complete eight-value claim-boundary set is required, every question resolves exactly once into exactly one output section, and authored order is preserved for questions, sections, section references, and claim boundaries. Free text is NFC, single-line, byte-bounded data. Version 1 uses fixed factual, inventory, dependency, scope, compatibility, budget, blocker, and recommendation sentence templates; each subject slot accepts only an exact template-specific phrase from a closed allowlist. Titles accept only exact safe subject phrases plus fixed static-analysis head nouns. Questions have exactly one final question mark, colon-bearing role or URI forms are outside the character grammar, and dotted text is rejected except for the bounded product-version template. Unicode control and format characters, compiler markers, approval or authorization token shapes, unresolved interpolation, template delimiters, structural prompt punctuation, structural Markdown, raw HTML, JSON containers, executable or shell syntax, host or URI sidecars, role or approval influence, secrets or environment requests, imperative directives, and prompt-meta instructions therefore remain outside the accepted language and fail with sanitized categorical diagnostics. Ordinary descriptive inspection questions represented by the closed templates remain valid, including `open`, `find`, `make`, `produce`, passive `read` or `written`, `Go`, and the bounded `Next.js` product-version form.

Presence of the contract requires `STATIC_INSPECTION_ONLY` authority and zero effects: no create or modify paths, commands, external resources, compatibility adapters, conditional scopes, rollback, target confirmation, Git mutation, governance transition, state transition, or nonzero operation budget. Preserve, exclude, and bounded read-content scope may still identify static evidence. The contract is never encoded as an executable command, a general prompt field, an arbitrary template, a sidecar authority artifact, or an initial state.

The fixed Codex and README renderers emit the title, ordered questions, required CCR sections, and claim boundaries. The CCR template emits those required sections plus an exact question coverage ledger. The complete normalized contract is embedded in the canonical phase spec, Authority IR, token basis, manifest identities, and checksum graph. The independent bundle verifier reconstructs the contract, the complete boundary set, and the zero-effect authority predicate from canonical evidence; it also reconstructs the README summary and CCR-specific sentinels with exact leading and trailing line boundaries and rejects altered, omitted, reordered, prefixed, or suffixed rendered content. Malformed direct-helper input fails with a stable categorical diagnostic rather than a raw exception. Stable diagnostics are `INSPECTION_CONTRACT_REFERENCE_INVALID`, `INSPECTION_AUTHORITY_MISMATCH`, and `INSPECTION_TEXT_FORBIDDEN`.
...
1. `phase-compiler.test.mjs` covers strict schemas, canonicalization, snapshot collection, semantic/governance/operation validation, dependency closure, P01-P04, and N01-N28.
2. `phase-compiler-security.test.mjs` covers new-only output, confinement, link/mode/race/tamper defenses, inspection-text injection rejection, P04 independent verification, ZIP binding, CLI behavior, sanitized failures, and cleanup.
3. `phase-compiler-determinism.test.mjs` compiles the same synthetic phases twice in separate `/tmp` destinations, compares all nine canonical files byte-for-byte, binds P04 ordering and approval sensitivity, rejects N28 rendered-content tampering, and proves that optional ZIP transport changes no canonical identity.
